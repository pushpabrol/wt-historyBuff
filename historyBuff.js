...
// Extend AlexaSkill
HistoryBuffSkill.prototype = AlexaSkill.prototype;
...
HistoryBuffSkill.prototype.eventHandlers.onSessionStarted = function(sessionStartedRequest, session) {
  //  console.log("HistoryBuffSkill onSessionStarted requestId: " + sessionStartedRequest.requestId + ", sessionId: " + session.sessionId);
...
module.exports = function(context, req, res) {
  ...
  console.log(context);
  var required_params = ['api_signing_secret'];
  for (var p in required_params)
    if (!context.secrets[required_params[p]]) {
      res.statusCode = 400;
      res.end('The `' + required_params[p] + '` must be provided.');
      return;
  }
  var api_signing_secret = context.secrets.api_signing_secret;

  if (context.data.session && context.data.session.user && context.data.session.user.accessToken) {
    var access_token = context.data.session.user.accessToken;
    console.log(access_token);
    jwt.verify(access_token, api_signing_secret, function(err, decoded) {

      if (err) {
        console.log(err);
        res.statusCode = 500;
        res.end(JSON3.stringify(err));
      }
      // here - In an actual application you
      // use the scope to check authorization
      console.log("Scope in token: " + decoded.scope);
      var skill = new HistoryBuffSkill();
      skill.execute(context.data, cb);

    });
  } else {
    res.statusCode = 400;
    res.end({
      "error": "Bad Request missing access_token"
    });
  }
  ...
  function cb(err, response) {
    ...
    if (err) {
      ...
      res.statusCode = 200;
      res.end(JSON3.stringify(response));
    }
